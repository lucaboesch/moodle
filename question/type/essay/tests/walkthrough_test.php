...
class qtype_essay_walkthrough_testcase extends qbehaviour_walkthrough_test_base {
    ...
    /**
     * Helper method: Check that a textarea does contain a given string.
     *
     * @param string $name textarea name.
     * @param string $content string content.
     * @param int $height textarea height.
     */
    protected function check_textarea_contains($name, $content = '', $height = 10) {
    ...
    /**
     * Helper method: Check that a textarea does not contain a given string.
     *
     * @param string $name textarea name.
     * @param string $content string content.
     * @param int $height textarea height.
     */
    protected function check_textarea_does_not_contain($name, $content = '', $height = 10) {
        $fieldname = $this->quba->get_field_prefix($this->slot) . $name;
        ...
        if ($content) {
            $this->assertNotRegExp('/' . preg_quote(s($content), '/') . '/', $this->currentoutput);
        }
    }
    ...
    public function test_deferred_feedback_html_editor() {
        ...
        $this->render();
        $this->check_textarea_contains('answer', '');
        ...
        $this->render();
        $this->check_textarea_contains('answer', $response);
        $this->check_current_output(
                $this->get_contains_question_text_expectation($q),
                $this->get_does_not_contain_feedback_expectation());
        $this->check_step_count(2);
    ...
    public function test_deferred_feedback_plain_text_responsetemplate() {

        // Create an essay question.
        $q = test_question_maker::make_question('essay', 'plainresponsetemplate');
        $this->start_attempt_at_question($q, 'deferredfeedback', 1);
        ...
        $this->check_current_mark(null);
        $this->render();
        $this->check_textarea_does_not_contain('answer', '<br>');
        $this->check_current_output(
            $this->get_contains_question_text_expectation($q),
            $this->get_does_not_contain_feedback_expectation());
        $this->check_step_count(1);
    ...
    public function test_responsetemplate() {
        ...
        $this->check_current_mark(null);
        $this->render();
        $this->check_textarea_contains('answer', 'Once upon a time');
        $this->check_current_output(
                $this->get_contains_question_text_expectation($q),
                $this->get_does_not_contain_feedback_expectation());
        $this->check_step_count(1);
        ...
        $this->check_step_count(2);
        $this->render();
        $this->check_textarea_contains('answer', 'Once upon a time there was a little green frog.');
        $this->check_current_output(
                $this->get_contains_question_text_expectation($q),
                $this->get_does_not_contain_feedback_expectation());
        $this->check_step_count(2);
